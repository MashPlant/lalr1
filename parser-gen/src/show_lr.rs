...
pub fn table(orig_table: &Table, table: &Table, g: &Grammar) -> String {
  ...
  for (idx, (o, n)) in orig_table.iter().zip(table.iter()).enumerate() {
    ...
    for item in o.closure { // o and n have the same items
      let _ = writeln!(text, "  {}", g.show_prod(item.prod_id as usize, Some(item.dot)));
    }
    text.push('\n');
    ...
    for (ch, ao) in &o.act {
      ...
      for o in ao {
        ...
        let _ = writeln!(text, "  {} => {:?} ({})", g.show_token(*ch as usize), o, keep);
      }
    }
    text.push('\n');
  }
...
fn show_link(g: &Grammar, link: &HashMap<u32, u32>, idx: usize, s: &mut String) {
  ...
  for (k, v) in link {
    let _ = writeln!(s, r#"{} -> {} [label="{}"];"#, idx, v, g.show_token(k as usize));
  }
}
...
pub fn lr0_dot(g: &Grammar, lr0: &Lr0Fsm) -> String {
  ...
  for (idx, Lr0Node { closure, link }) in lr0.iter().enumerate() {
    ...
    for lr0 in closure {
      text += &g.show_prod(lr0.prod_id as usize, Some(lr0.dot));
      text.push(',');
      text.pop();
      text += r#"\n"#;
    }
