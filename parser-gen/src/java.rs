...
use common::{grammar::*, re2dfa::Dfa, *};
use crate::{Config, fmt};

impl<W: std::io::Write> Config<'_, W> {
  pub fn java_lalr1(&mut self, g: &Grammar, table: &Table, dfa: &Dfa) {
    ...
    let _ = write!(
      ...
      stack_item = fn2display(move |f| for (i, ty) in types.iter().enumerate() {
      ...
      parser_act = fn2display(move |f| for (i, prod) in g.prod.iter().enumerate() {
